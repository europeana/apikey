...
public class ApiKeyController {
    ...
    public ResponseEntity<Object> createKeyAndClient(@RequestBody ApiKeyRequest newKeyRequest) throws ApiKeyException {
        ...
        ClientRepresentation newClientRep =  keycloakClientManager.recreateClient(securityContext, newApiKey.getApiKey(), requestClient);

        // do something useful with this thing
        

        ////

//        this.apiKeyRepo.save(new ApiKey(newKeyData));
//        LOG.debug("API key {} created", newKeyData.getApiKey());
//
...
//        return new ResponseEntity<>(apiKey, HttpStatus.CREATED);
     ...
     */
    @PostMapping(path = "/{apiKey}/addclient")
    public ResponseEntity<HttpStatus> addClient(@PathVariable String apiKey) throws ApiKeyException {
        ...
        ClientRepresentation newClientRep = createLinkedClient(kcAuthToken, clientKey);
    ...
    @PutMapping(value = "/{id}",
                produces = MediaType.APPLICATION_JSON_VALUE,
                consumes = MediaType.APPLICATION_JSON_VALUE)
    public ApiKey update(@PathVariable("id") String id, @RequestBody ApiKeyRequest updateKeyRequest) throws
                                                                                                     ApiKeyException {
        KeycloakAuthenticationToken kcAuthToken = checkManagerCredentials();
    ...
    public ResponseEntity<Object> disable(@PathVariable("id") String id) throws ApiKeyException {
        KeycloakAuthenticationToken kcAuthToken = checkManagerCredentials();
        String keyCloakId = keycloakClientManager.checkifClientExists(id,
    ...
    private ClientRepresentation createLinkedClient(KeycloakAuthenticationToken kcAuthToken, ApiKey clientKey) throws
                                                                                                               ApiKeyException {
        KeycloakSecurityContext securityContext = (KeycloakSecurityContext) kcAuthToken.getCredentials();
        ApiKeyRequest           requestClient   = copyValuesToNewApiKeyRequest(clientKey);
        return keycloakClientManager.recreateClient(securityContext, clientKey.getApiKey(), requestClient);
    }
